...
- 状態数調査: `cargo run --release --example frontier_counts -- 8 raw` または `cargo run --release --example frontier_counts -- 8 normalized`
...
- `examples/frontier_counts.rs`: `raw` / `normalized` モードで手数ごとの一意な盤面キー数を数える実験。過去手数とは重複排除しない。
- `docs/adr/`: 設計判断の記録。
...
- 正規化キー: `normalized_key` が返す `u128`。盤面を8通りの対称変換（z軸中心の回転4通り × x座標反転2通り）に対して正規化し、最小値を返す。重力があるため z軸反転は含めない。評価表を引くためのキーであり、実際の着手座標は正規化前の盤面から扱う。
- `Outcome`: 探索中の局面を、次に手を打つプレイヤーから見た結果。現時点では手数を持たない。`GameStatus::InProgress` はまだ `Outcome` ではない。子局面の結果は `flip()` で親局面の視点に戻す。
- `MemoTable`: 正規化済み盤面キーから `Outcome` を取り出すメモ化用の表。回転・鏡映で同じ局面は同じエントリに収束する。
...
- ADR-0006 盤面の正規化キーで対称性を吸収する（z軸回転4通り × x座標反転2通り = 8通り）
